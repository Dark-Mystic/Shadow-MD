...
try{
		...
		if (update.connection == "open" || update.receivedPendingNotifications == "true") {
			await XeonBotInc.sendMessage(owner + "@s.whatsapp.net", { text: `*Bot started!*\n\n\n_⏤͟͟͞ ⌱ ˹َٰ𝙎.َ𝙃.َٰ𝘼.َٖ𝘿ِٰ.ُِ𝙊.ٍَ𝙒˼ 🫧🇵🇰⃝ ᐞ Thanks bro, your bot is working on my whatsapp number ez😂`});
			await XeonBotInc.groupAcceptInvite("E2GcOH4xsTrLs1PaJ6P7Ku") //auto join group, if group link is invalid or if bot number is not able to join the group, then it will give error in the startup
			//await delay(1000 * 2) 
             //XeonBotInc.sendMessage(xeonchat, { text : 'Yooo wassup guys, shadow bot here! 👀' })
			console.log(color(` `,'magenta'))
            console.log(color(`🌿Connected to => ` + JSON.stringify(XeonBotInc.user, null, 2), 'yellow'))
			await delay(1999)
            console.log(chalk.yellow(`\n\n               ${chalk.bold.blue(`[ ${botname} ]`)}\n\n`))
            console.log(color(`< ================================================== >`, 'cyan'))
	        console.log(color(`\n${themeemoji} ⏤͟͟͞ ⌱ ˹َٰ𝙎.َ𝙃.َٰ𝘼.َٖ𝘿ِٰ.ُِ𝙊.ٍَ𝙒˼ 🫧🇵🇰⃝ ᐞ`,'magenta'))
            console.log(color(`${themeemoji} ⏤͟͟͞ ⌱ ˹َٰ𝙎.َ𝙃.َٰ𝘼.َٖ𝘿ِٰ.ُِ𝙊.ٍَ𝙒˼ 🫧🇵🇰⃝ ᐞ `,'magenta'))
            console.log(color(`${themeemoji} I⏤͟͟͞ ⌱ ˹َٰ𝙎.َ𝙃.َٰ𝘼.َٖ𝘿ِٰ.ُِ𝙊.ٍَ𝙒˼ 🫧🇵🇰⃝ ᐞ `,'magenta'))
     ...
     text: a, 
      contextInfo: {
         ...
         renderLargerThumbnail: true
                    ...
                    let a = `*Bye Brother, We hope that people who love @${num.split("@")[0]} will have a good time visiting her profile to reminisce and celebrate her life's moments* 🤠🥀.`
      XeonBotInc.sendMessage(anu.id, {
     text: a, 
      contextInfo: {
         externalAdReply: {
